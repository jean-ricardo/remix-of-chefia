...
function DashboardPageContent() {
  ...
  const search = useSearch({ from: "/" }) as { taskId?: string };
  ...
  const todayCompletions = completions.filter((c) =>
    c.completed_at ? isToday(new Date(c.completed_at)) : false
  );
            ...
            <div className="space-y-6">
              {loadingCompletions ? (
                ...
                todayCompletions.slice(0, 5).map((completion) => {
                  ...
                  const completedDate = completion.completed_at ? new Date(completion.completed_at) : new Date();
                  return (
                    <div key={completion.activity_id + completion.completed_at} className="relative flex gap-4 pb-6 last:pb-0">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Concluída às {format(completedDate, "HH:mm")}
                        </p>
                      </div>
                    </div>
                  );
