...
begin
    ...
    -- 3. Limpeza de dados (Garantir que referências em outras tabelas não bloqueiem a exclusão)
    -- Deleta atividades atribuídas ao usuário
    delete from public.activities where assigned_user_id = target_user_id;
    -- Deleta registros vinculados na team_members
    delete from public.team_members where id = target_user_id;

    -- 4. Exclusão definitiva da conta no Supabase Auth
